...
comp_sp_summ <- comp_tb %>% group_by(PLOT, Species) %>% 
  summarise(BA_1935 = sum(DBH_1935^2 * 0.00007854 * 9.88, na.rm = T),
            ...
            TPH_1935 = sum(TPH_1935 * 8, na.rm = T),
            TPH_1940 = sum(TPH_1940 * 8, na.rm = T),
            TPH_1958 = sum(TPH_1958 * 8, na.rm = T),
            TPH_1964 = sum(TPH_1964 * 8, na.rm = T),
            TPH_1973 = sum(TPH_1973 * 8, na.rm = T),
            TPH_1978 = sum(TPH_1978 * 8, na.rm = T),
            TPH_1983 = sum(TPH_1983 * 8, na.rm = T),
            TPH_1997 = sum(TPH_1997 * 8, na.rm = T),
            TPH_2012 = sum(TPH_2012 * 8, na.rm = T),
            TPH_2023 = sum(TPH_2023 * 8, na.rm = T)) %>% 
  mutate(
...
```{r Species basal area (plot-level), include=FALSE}
plot_sp_summ <- comp_sp_summ %>% 
    mutate(Species = as.factor(case_match(Species,
             'yellow-poplar' ~ 'yellow-poplar',
             'white oak' ~ 'white oak',
             'sugar maple' ~ 'sugar maple',
  ...
  summarise(across(BA_1935:TPH_2023, ~ sum(.x, na.rm = T))) %>% 
  ungroup() %>% 
  #select(Species, starts_with('BA_')) %>% 
  pivot_longer(!c(PLOT, Species), names_to = 'Metric', values_to = 'Value') %>% 
  mutate(Year = as.integer(str_split_i(Metric, '_', 2)),
         Metric = str_split_i(Metric, '_', 1))  

```
### Plot 1
```{r Plot 1, echo=FALSE}
plot_sp_summ %>% filter(PLOT == 1, Metric == 'BA') %>%   
plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
          hovertemplate = paste("<b>%{text}</b><br>",
                                "%{xaxis.title.text}: %{x}<br>",
                                "%{y:.1f} m<sup>2</sup> ha<sup>-1</sup>",
    ...
    layout( 
      xaxis = list(title = 'Year', showgrid = F, showline = T),
      yaxis = list(title = 'Basal Area (m<sup>2</sup> ha<sup>-1</sup>)', showgrid = F, showline = T),
      legend = list(title = list(text='Species'), orientation = 'h', y = -0.2)
      ) 

plot_sp_summ %>% filter(PLOT == 1, Metric == 'TPH') %>%
  plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
                    hovertemplate = paste("<b>%{text}</b><br>",
                      "%{xaxis.title.text}: %{x}<br>",
...
### Plot 2
```{r Plot 2, echo=FALSE}
plot_sp_summ %>% filter(PLOT == 2, Metric == 'BA') %>%   
plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
          hovertemplate = paste("<b>%{text}</b><br>",
                                "%{xaxis.title.text}: %{x}<br>",
...
plot_sp_summ %>% filter(PLOT == 2, Metric == 'TPH') %>%
  plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
                    hovertemplate = paste("<b>%{text}</b><br>",
                      "%{xaxis.title.text}: %{x}<br>",
    ...
    layout( 
      xaxis = list(title = 'Year', showgrid = F, showline = T),
      yaxis = list(title = 'Stem Density (trees ha<sup>-1</sup>)', showgrid = F, showline = T),
      legend = list(title = list(text='Species'), orientation = 'h', y = -0.2)
      )
```
...
### Plot 3
```{r Plot 3, echo=FALSE}
plot_sp_summ %>% filter(PLOT == 3, Metric == 'BA') %>%   
plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
          hovertemplate = paste("<b>%{text}</b><br>",
                                "%{xaxis.title.text}: %{x}<br>",
...
plot_sp_summ %>% filter(PLOT == 3, Metric == 'TPH') %>%
  plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
                    hovertemplate = paste("<b>%{text}</b><br>",
                      "%{xaxis.title.text}: %{x}<br>",
...
### Plot 4
```{r Plot 4, echo=FALSE}
plot_sp_summ %>% filter(PLOT == 4, Metric == 'BA') %>%   
plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
          hovertemplate = paste("<b>%{text}</b><br>",
                                "%{xaxis.title.text}: %{x}<br>",
...
plot_sp_summ %>% filter(PLOT == 4, Metric == 'TPH') %>%
  plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
                    hovertemplate = paste("<b>%{text}</b><br>",
                      "%{xaxis.title.text}: %{x}<br>",
...
### Plot 5
```{r Plot 5, echo=FALSE}
plot_sp_summ %>% filter(PLOT == 5, Metric == 'BA') %>%   
plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
          hovertemplate = paste("<b>%{text}</b><br>",
                                "%{xaxis.title.text}: %{x}<br>",
...
plot_sp_summ %>% filter(PLOT == 5, Metric == 'TPH') %>%
  plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
                    hovertemplate = paste("<b>%{text}</b><br>",
                      "%{xaxis.title.text}: %{x}<br>",
...
### Plot 6
```{r Plot 6, echo=FALSE}
plot_sp_summ %>% filter(PLOT == 6, Metric == 'BA') %>%   
plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
          hovertemplate = paste("<b>%{text}</b><br>",
                                "%{xaxis.title.text}: %{x}<br>",
...
plot_sp_summ %>% filter(PLOT == 6, Metric == 'TPH') %>%
  plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
                    hovertemplate = paste("<b>%{text}</b><br>",
                      "%{xaxis.title.text}: %{x}<br>",
...
### Plot 7
```{r Plot 7, echo=FALSE}
plot_sp_summ %>% filter(PLOT == 7, Metric == 'BA') %>%   
plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
          hovertemplate = paste("<b>%{text}</b><br>",
                                "%{xaxis.title.text}: %{x}<br>",
...
plot_sp_summ %>% filter(PLOT == 7, Metric == 'TPH') %>%
  plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
                    hovertemplate = paste("<b>%{text}</b><br>",
                      "%{xaxis.title.text}: %{x}<br>",
...
### Plot 8
```{r Plot 8, echo=FALSE}
plot_sp_summ %>% filter(PLOT == 8, Metric == 'BA') %>%   
plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
          hovertemplate = paste("<b>%{text}</b><br>",
                                "%{xaxis.title.text}: %{x}<br>",
...
plot_sp_summ %>% filter(PLOT == 8, Metric == 'TPH') %>%
  plot_ly(x = ~Year, y = ~Value, color = ~Species, text = ~Species,
          type = 'scatter', mode = 'line', stackgroup = 'one',
                    hovertemplate = paste("<b>%{text}</b><br>",
                      "%{xaxis.title.text}: %{x}<br>",
